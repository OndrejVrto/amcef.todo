...
@section('content')
    <div class="col-lg-10 col-xl-9 mx-auto">
        <h2 class="text-end">Edit task</h2>

        <form action="{{ action(\App\Http\Controllers\Task\SaveTaskController::class, $task) }}" method="POST">
            @csrf
            ...
            <div class="g-1">
                ...
                <div class="row mb-3">
                        ...
                        <select name="status" class="form-select @error('status') is-invalid @enderror">
                            ...
                            @foreach ($taskStatuses as $taskStatus)
                                <option
                                    ...
                                    @selected(old('status', $task->status->value) == $taskStatus->value)
                                >
                                    {{ $taskStatus->label() }}
                                </option>
                            @endforeach
                    ...
                    <div class="col-lg-6">
                        ...
                        <input
                            ...
                            value="{{ old('deadline_until', $task->deadline_until->format('Y-m-d')) }}"
                        >
                ...
                <div class="mb-3">
                    ...
                    <textarea name="description" rows="3" class="form-control @error('description') is-invalid @enderror">{{ old('description', $task->description) }}</textarea>
                    @error('description')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
